...
```{r}
deserts2 <- deserts %>% mutate(year=as_factor(year))   #change year into factor so ggplot lists ALL the years in the graph
deserts2
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "lavenderblush3", bins=40)+
...
```{r}
homerange %>% 
  ggplot(aes((y = ..density..),x = log10.hra)) +
  geom_density(alpha = 0.4, color = "black", fill = "lavenderblush3", bins=40)+
  labs(title = "Distribution of Homerange")
```
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_histogram(aes(y = after_stat(density)), fill = "lavenderblush", alpha = 0.4, color = "black") +
  geom_density(color = "red") +
  labs(title = "Distribution of Homerange")
```
...
homerange %>% 
  ...
  ggplot(aes(x=class, y=log10.mass, fill= class))+
         geom_boxplot() +
         labs(title = "Small Range Category by Class",
              x= "Class",
              y= "log10 Mass")
```
