...
```
```{r}
glimpse(spider_list$`Santa Clara`)
```
...
inner_exampleDF   #only included the customers in common that has all data (merged variables)
```
...
left_exampleDF    #added the unique variables from Table B to Table A (but did not add new customers)
```
...
right_exampleDF    #added the unique variables from Table A to Table B (but did not add new customers)
```

`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID")
full_exampleDF    #joined all variables and combined all customers
```
