...
my_experiment
```
 add another "my_experiment" to check and to show when you knit
...
```
Concatenate all the values to make sure the mean function doesn't round!!!
...
I think this means that there is a missing value in the data set, so NA is used as a placeholder. The mean calculation could not be done with the placeholder 'NA' present in the data.
...
```
This is useful because this removes the NA placeholder, allowing the mean calculation to take place.
