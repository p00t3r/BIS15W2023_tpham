...
#csv = comma seperated value (a file type) used internationally
...
```
#Change variable scientist to factor (change class) - important b/c some packages heavily rely on specific class types
```{r}
...
#windows users: turn the forntslashes to backslashes
```{r}
setwd("/Users/pooter/Desktop/BIS15W2023_tpham/lab3/")
getwd()
```
...
5. . Try the `table()` command to produce counts of mammal order, family, and genus.  #one table at a time
```{r}
table(mammals$order)
```
```{r}
