...
The largest deer has a log10 mass of 5.49. Its common name is the moose.
...
```r
snakes_mass <- select(snakes,"mean.hra.m2", "common.name")
snakes_mass
```
...
```r
arrange(snakes_mass, mean.hra.m2)
```
...
## # … with 31 more rows
```
The snake species with the smallest home range is the namaqua dwarf adder. They are considered mildly venomous and threatened (in terms of conservation status) due to mining and illegal collecting.
