...
```r
superhero_info <- readr::read_csv("data/heroes_information.csv", na = c("", "-99", "-"))        #replace space, -99, or -, with NA
```
...
```r
superhero_info_new <- rename(superhero_info, gender="Gender", eye_color="Eye color", race= "Race", hair_color="Hair color", height="Height", publisher="Publisher", skin_color="Skin color", alignment="Alignment", weight="Weight")
superhero_info_new
```
...
```
There are more bald people among the good guys.
...
## #   ⁴​skin_color, ⁵​alignment
```
We don't have 16 rows in Q10 because we used " | " to denote that we want to filter superheroes that have a height over 200 or weight greater than or equal to 450. Using " & " instead would denote we want superheroes that have a height over 200 and weight greater than or equal to 450, resulting in less rows because it is more specific.
...
## #   ⁴​skin_color, ⁵​alignment
```
Flora Colossus has the highest height to weight ratio by far, at over 175.
...
## #   stamina <lgl>, astral_travel <lgl>, audio_control <lgl>, dexterity <lgl>, …
```
There are 97 superheroes that have a combination accelerated healing, durability, and super strength.
...
```
My bae Black Widow has agility, stealth, marksmanship, longevity, intelligence, stamina, and others. It's too bad one of her powers isn't plot armor because she died. #sad_face_emoji
