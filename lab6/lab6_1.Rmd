...
msleep24 <- msleep %>% 
  ...
  filter(!is.na(vore)) %>%     #removing NA from variables
...
```
```{r}
glimpse(msleep) #vore is a character, so we can use tabyl
```
...
```{r}
#NA resembles missing information. The NA's overwrite all the data form that order for brain weight, yielding in a NA for the mean value. 
```
...
msleep %>% 
  ...
  select(order, genus, brainwt)    # the NaN means that all the data points there are missing!!
```
