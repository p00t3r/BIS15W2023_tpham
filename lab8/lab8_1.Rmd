...
```{r}
heartrate <- read_csv(here("lab8", "data2", "heartrate.csv"))    #point where we goin from 'here'
```

```{r}
heartrate    #drugs shift from wide to short format
#plotting won't work on wide data
```
...
```{r}
#These data are untidy because the categories of income should be observations rather than variables.
```
...
```{r}
#Day number should be in a single column.
```
