...
surgery %>% 
  ...
  group_by(ahrq_ccs) %>% 
  summarize("Top 5 Procedures associated with highest risk of 30-day mortality" = slice_max("ccsmort30rate"))
```
```{r}
surgery %>% 
  tabyl(ahrq_ccs)
...
surgery %>% 
  group_by(month) %>% 
  summarise(ccsmort30rate, ccscomplicationrate) %>% 
...
```{r}
surgery %>% 
  ggplot(aes(y= ccsmort30rate, x=month)) +
  geom_col() +
  scale_x_discrete(limits = month.abb) +
  labs(title = "Overall Mortality Rate by Month",
       x= "Month",
       y= "Mortality Rate") 
```
```{r}
surgery %>% 
  ggplot(aes(y= ccscomplicationrate, x=month)) +
  geom_col() +
  scale_x_discrete(limits = month.abb) +
  labs(title = "Overall Complication Rate by Month",
       x= "Month",
       y= "Complication Rate") 
```
